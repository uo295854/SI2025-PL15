...
public class ListadoCargosMensualesModel {
	...
	public List<CargosMensualesDTO> getListadoCargosMensuales(int año, String mes){
		...
		String SQL = "SELECT \r\n"
		        ...
		        + "\r\n"
		        + "    COALESCE(SUM(CASE \r\n"
		        + "        WHEN p.concepto = 'RESERVA' AND p.estado = 'PENDIENTE' THEN p.importe \r\n"
		        + "        WHEN p.concepto = 'RESERVA' AND p.estado = 'DEVUELTO' THEN -p.importe \r\n"
		        + "        ELSE 0 \r\n"
		        + "    END), 0) AS cargosReservas,\r\n"
		        + "\r\n"
		        + "    COALESCE(SUM(CASE \r\n"
		        + "        WHEN p.concepto = 'INSCRIPCION' AND p.estado = 'PENDIENTE' THEN p.importe \r\n"
		        + "        WHEN p.concepto = 'INSCRIPCION' AND p.estado = 'DEVUELTO' THEN -p.importe \r\n"
		        + "        ELSE 0 \r\n"
